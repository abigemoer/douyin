...
class Douyin():
    ...
    def login(self):
        ...
        self.check(page)
    def check(self,page):
        choose=['青衫夜读','蟹老板真的会谢','杨一面对面','小五哥哥']
        for m in choose:
            # 清空搜索框
            ...
            tab.ele('x://*[@id="search-content-area"]/div/div[1]/div[1]/div/div[1]/div/span[3]',timeout=50).click()
            tab2=page.latest_tab
            # 找到第一个目标，获取用户主页链接
            #href=tab2.ele('x://*[@class="search-result-card"]/a',timeout=50).attr('href')
            href=tab2.ele('x://*[@id="search-result-container"]/div[3]/ul/li[1]/div/a').attr('href')
            name=tab2.ele('x://*[@id="search-result-container"]/div[3]/ul/li[1]/div/a/div[1]/div/div/p/span/span/span/span/span/span').text
            print(name)
            number=tab2.ele('x:// *[ @ id = "search-result-container"]/div[3]/ul/li[1]/div/a/div[2]/span[1]/span').text
            print(number)
    ...
    def parse(self,page,sec_user_id,m):
        ...
        while result['has_more']==1:
            ...
            for i in range(length):
                ...
                df.to_csv('{}.csv'.format((m)),index=False,encoding='utf-8-sig')
    def inits(self):
        m=input("是否登录，0or1\n")
        if m=='0':
            self.login()
